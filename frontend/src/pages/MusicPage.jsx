  ...
  async function handleGenerate(e) {
    ...
    try {
      const data = await apiFetch('/music/sessions', {
        method: 'POST',
        body: JSON.stringify({ ...form, output_format: 'abc' }),
      })
      setSession(data)
  ...
  return (
    <div className="music-layout">
      ...
      <aside className="music-sidebar">
        <h2 className="section-title">產生 Walking Line</h2>
        <form onSubmit={handleGenerate} className="music-form">
          <div className="field">
            <label>調性</label>
        ...
        {session && (
          <div className="refinement-section">
            <h3 className="section-title" style={{ marginTop: 24 }}>精修</h3>
            <form onSubmit={handleRefine} className="refinement-form">
              ...
              <div className="refinement-actions">
                <button type="submit" className="btn-primary" disabled={loading || !refinementText.trim()}>
                  {loading ? '精修中…' : '精修'}
                </button>
      ...
      {/* Right: result */}
      <section className="music-result">
        {!session ? (
          ...
          <>
            ...
            {currentPiece && (
              <div className="piece-display">
                ...
                {/* ABC notation rendered as sheet music */}
                {currentPiece.notation ? (
                  <div className="notation-section">
                    <AbcRenderer notation={currentPiece.notation} />
                  </div>
                ) : (
                  <p className="text-muted" style={{ marginBottom: 16 }}>（此版本無 ABC notation）</p>
                )}
